classdef GraspFrictionConeWrench < RigidBodyContactWrench
  ...
  properties(SetAccess = protected)
    ...
    grasp_object_FC_axis % A 3 x obj.num_pts vector. The normal direction in the GRASP OBJECT frame
  ...
  methods
    ...
    function [normal_dir,dnormal_dir] = normalDirection(obj,kinsol)
      % returns the normal direction in the WORLD frame
      [grasp_pos,dgrasp_pos] = forwardKin(obj.robot,kinsol,obj.grasp_object_idx,[[0;0;0],obj.grasp_object_FC_axis],0);
      normal_dir = grasp_pos(:,2:end)-bsxfun(@times,grasp_pos(:,1),ones(1,obj.num_pts));
      dnormal_dir = dgrasp_pos(4:end,:)-repmat(dgrasp_pos(1:3,:),obj.num_pts,1);
    end
    
    function [pos,J] = bodyContactPosition(obj,kinsol)
      % return the position of the contact in the OBJECT frame
      % @retval pos   -- A 3 x obj.num_pts matrix. pos(:,i) is the position of the i'th
      % contact point in the OBJECT frame
      [finger_pos,dfinger_pos] = forwardKin(obj.robot,kinsol,obj.finger_idx,obj.finger_grasp_pt,0);
      [object_pose,dobject_pose] = forwardKin(obj.robot,kinsol,obj.grasp_object_idx,[0;0;0],2);
      [R_object,dR_object] = quat2rotmat(object_pose(4:7));
      dR_object = dR_object*dobject_pose(4:7,:);
      pos_diff = finger_pos-bsxfun(@times,object_pose(1:3),ones(1,obj.num_pts));
      pos = R_object'*pos_diff;
      dpos_diff = dfinger_pos-repmat(dobject_pose(1:3,:),obj.num_pts,1);
      J = matGradMultMat(R_object',pos_diff,transposeGrad(dR_object,[3,3]),dpos_diff);
    end
    
    function [pos_t,dpos_t] = tangentialDisplacement(obj,kinsol1,kinsol2)
      % return the tangential displacement in the OBJECT frame.
      [pos1,J1] = bodyContactPosition(obj,kinsol1);
      [pos2,J2] = bodyContactPosition(obj,kinsol2);
      pos_diff = pos1-pos2;
      dpos_diff = [J1 -J2];
      pos_diff_normal = bsxfun(@times,sum(obj.grasp_object_FC_axis.*pos_diff,1),ones(3,1)).*obj.grasp_object_FC_axis;
      dpos_diff_normal = kron(eye(obj.num_pts),ones(3,3)).*(obj.grasp_object_FC_axis(:)*obj.grasp_object_FC_axis(:)')*dpos_diff;
      pos_t = pos_diff-pos_diff_normal;
      dpos_t = dpos_diff-dpos_diff_normal;
    end
  end
