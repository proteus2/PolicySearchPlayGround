...
%       obj = obj.setSolverOptions('snopt','scaleoption',2);
      ...
      for i = 1:obj.N-1
        q_cost = FunctionHandleConstraint(-inf,inf,1+obj.nq,@(h,q) qCost(Q,q_nom(:,i),h,q),1);
        obj = obj.addCost(q_cost,{obj.h_inds(i);obj.q_inds(:,i)});
      end
      ...
      com_accel_cost = FunctionHandleConstraint(-inf,inf,4,@(h,comddot) comAccelCost(Q_comddot,h,comddot),1);
      for i = 1:obj.N-1
        obj = obj.addCost(com_accel_cost,{obj.h_inds(i);obj.comddot_inds(:,i)});
      end
      ...
      v_cost = FunctionHandleConstraint(-inf,inf,1+obj.nv,@(h,v) vCost(Qv,h,v),1);
      for i = 1:obj.N-1
      ...
      function [f,df] = comAccelCost(Q,h,comddot)
        ...
        df = [0.5*(comddot'*Q*comddot), h*comddot'*Q];
      end
      ...
      function [f,df] = qCost(Q,q_nom,h,q)
        f = 0.5*h*((q-q_nom)'*Q*(q-q_nom));
        df = [0.5*(q-q_nom)'*Q*(q-q_nom), h*(q-q_nom)'*Q];
      end
    end
  ...
  methods(Access = protected)
    ...
    function obj = addCentroidalDynamicConstraints(obj)
      ...
      for i = 1:obj.N
        com_cnstr = FunctionHandleConstraint(zeros(3,1),zeros(3,1),obj.nq+3,@(~,com,kinsol) comMatch(kinsol,com),1);
        ...
        H_cnstr = FunctionHandleConstraint(zeros(3,1),zeros(3,1),obj.nq+obj.nv+3,@(~,v,H,kinsol) angularMomentumMatch(kinsol,v,H),1);
    ...
    function obj = addContactDynamicConstraints(obj,knot_idx,contact_wrench_idx,knot_lambda_idx)
      ...
      if(num_knots == 1)
        ...
        Hdot_cnstr = FunctionHandleConstraint(zeros(3,1),zeros(3,1),obj.nq+3+num_lambda1+3,@(~,com,lambda,Hdot,kinsol) singleTimeDynFun(kinsol,com,lambda,Hdot),1);
      ...
      elseif(num_knots == 2)
        ...
        h_cnstr = FunctionHandleConstraint(zeros(9+obj.nq,1),zeros(9+obj.nq,1),1+24+2*obj.nq+obj.nv,@dtDynFun,1);
