classdef LinearFrictionConeWrench < RigidBodyContactWrench
  ...
  methods
    ...
    function [normal,dnormal] = normalDirection(obj,kinsol)
      % returns the normal direction at each of the contact points
      % @retval normal   A 3 x obj.num_pts matrix. normal(:,i) is the normal vector at
      % the i'th contact points
      % @retval dnormal  A (3*obj.num_pts)  x nq matrix. The gradient of normal w.r.t q
      FC_edge_norm = sqrt(sum(obj.FC_edge.^2,1));
      FC_edge_normalized = obj.FC_edge./bsxfun(@times,FC_edge_norm,ones(3,1));
      normal = sum(FC_edge_normalized,2);
      normal = bsxfun(@times,ones(1,obj.num_pts),normal/norm(normal));
      dnormal = sparse(3*obj.num_pts,obj.robot.getNumPositions());
    end
  end
