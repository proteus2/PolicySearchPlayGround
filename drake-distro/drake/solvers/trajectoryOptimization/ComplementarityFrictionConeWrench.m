  ...
  methods
    function obj = ComplementarityFrictionConeWrench(robot,body,body_pts,FC_mu,FC_axis,phi_handle,force_normalize_factor,ncp_tol)
      ...
      for i = 1:obj.num_slack
        obj.slack_name{i} = sprintf('%ground_s_friction_cone_complementarity_slack%d',obj.body_name,i);
      end
      ...
      for i = 1:obj.num_pts
        comp_name{i} = sprintf('%ground_s_pt%d_contact_distance-gamma=0',obj.body_name,i);
        comp_name{obj.num_pts+i} = sprintf('ground_<%s_pt%d_normal_force,gamma>=0',obj.body_name,i);
      end
