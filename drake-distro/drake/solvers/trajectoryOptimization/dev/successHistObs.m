function successHistObs
    n_obs = 40;
    n_trials = 1;
    success_score = zeros(n_obs,1);
    for obs_idx = 1:n_obs
            fname= sprintf('./observable_init/new_traj_%d.mat',obs_idx-1);
            load(fname);
    ...
    for obs_idx = 1:n_obs
        for trial_idx=1:n_trials
            fname= sprintf('./MMD_training_output_observable/MMD_predicted_traj_%d_%d.mat',obs_idx-1,trial_idx-1);
            load(fname);
...
%             qtraj_sol = PPTrajectory(foh(cumsum(h_sol), q_sol));
    ...
    for obs_idx = 1:n_obs
        for trial_idx=1:n_trials
            fname= sprintf('./sup_training_output_observable/MMD_predicted_traj_%d_%d.mat',obs_idx-1,trial_idx-1);
            load(fname);
            q_sol=xtraj(1:end-1,1:end);
%             qtraj_sol = PPTrajectory(foh(cumsum(h_sol), q_sol));
            success_score(obs_idx,trial_idx) = checkSuccess(q_sol);
    ...
    title('Success Rate for 40 Different Observable Models')
    ylim([0 1])
    ylabel('Success Rate')
    xlabel('Algorithms')
