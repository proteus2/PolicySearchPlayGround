function testRobustGrasp_multiple_num_samples
    checkDependency('lcmgl');
    %delete('ik_robust*');
    %delete('x_sol_num_samples*');
    while(1)
        clear classes        
        ik_files = dir('ik_robust*');
        
        if size(ik_files,1) == 0
            num_samples = 1;
        elseif str2num(ik_files(end).name(end))>=7
            break;
        else
            num_samples = str2num(ik_files(end).name(end))+2;
        end
        ...
        obj_end_pose = [0; 0.5; len/2; zeros(3,1)];
        ...
        plan = RobustGrasp(hand_urdf,hand_tip_names,...
          ...
          N,tf_range,Q_comddot,Qv,Q,q_nom, Q_contact_force, @(i) uncertainty(num_samples,radius,len,i),num_samples,ncp_tol);
        ...
        ik_robust_name = sprintf('ik_robust.out_%d',num_samples);
        plan = plan.setSolverOptions('snopt','print',ik_robust_name);
        ...
        ik_robust_cmd = sprintf('touch ik_robust.out_%d', num_samples);
        system(ik_robust_cmd);
        ...
        sol_name=sprintf('x_sol_num_samples_%d', num_samples);
        save(sol_name,'q_sol','v_sol','h_sol','time_took') ;
    end
end
...
function urdf = uncertainty(index,radius,len,count)
mass = rand()+1;
inertia = zeros(3);
...
urdf = sprintf('%d_cylinder%d.urdf',index,count);
fid = fopen(urdf,'w');
