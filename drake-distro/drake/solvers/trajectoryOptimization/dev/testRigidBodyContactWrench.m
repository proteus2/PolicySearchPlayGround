...
display('check CylinderFrictionConeWrench');
robot = robot.addRobotFromURDF('../dev/cylinder.urdf',[],[],struct('floating',true,'replace_cylinders_with_capsules',false));
cylinder_idx = robot.findLinkInd('cylinder');
l_hand_pt = randn(3,1);

cylinder_wrench = CylinderFrictionConeWrench(robot,cylinder_idx,l_hand,l_hand_pt,[0;0;1],randn(3,1),0.04,1);
q = randn(robot.getNumPositions(),1);
F = randn(3,1);
slack = randn();
testRigidBodyContactWrench_userfun(cylinder_wrench,q,F,slack);
end
...
kinsol = rb_wrench.robot.doKinematics(q);
if(~isa(rb_wrench,'GraspWrench'))
  % check normalDirection
  [normal_dir,dnormal_dir] = normalDirection(rb_wrench,kinsol);
  valuecheck(sum(normal_dir.*normal_dir,1),ones(1,rb_wrench.num_pts),1e-4);
  [~,dnormal_dir_numeric] = geval(@(q) normalDirection(rb_wrench,doKinematics(rb_wrench.robot,q)),q,struct('grad_method','numerical'));
  valuecheck(dnormal_dir,dnormal_dir_numeric,1e-4);
  % check bodyContactPosition
  kinsol = rb_wrench.robot.doKinematics(q);
  [pos,dpos] = bodyContactPosition(rb_wrench,kinsol);
  [~,dpos_numeric] = geval(@(q) bodyContactPosition(rb_wrench,doKinematics(rb_wrench.robot,q)),q,struct('grad_method','numerical'));
  valuecheck(dpos,dpos_numeric,1e-4);
  q1 = q;
  kinsol1 = rb_wrench.robot.doKinematics(q1,false,false);
  q2 = q+randn(nq,1);
  kinsol2 = rb_wrench.robot.doKinematics(q2,false,false);
  [pos_t,dpos_t] = tangentialDisplacement(rb_wrench,kinsol1,kinsol2);
  [~,dpos_t_numeric] = geval(@(x) tangentialDisplacement(rb_wrench, doKinematics(rb_wrench.robot,x(1:nq),false,false),doKinematics(rb_wrench.robot,x(nq+(1:nq)),false,false)),[q1;q2],struct('grad_method','numerical'));
  valuecheck(dpos_t,dpos_t_numeric,1e-4);
end
end
