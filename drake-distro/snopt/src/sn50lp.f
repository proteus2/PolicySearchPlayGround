...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &     nDegen, lencw, leniw, lenrw, subopt, hEtype(nb),
     ...
     &     gBS(m+1), pi(m), y(nb), y1(nb), rw(lenrw)
      character
...
!     ==================================================================
     ...
     &     jstFea, justPh1, LUok, needf, needv, needpi, newB, newLU,
     ...
     &     nElast, nFac, nFreez, nInfE, nonOpt, nSwap, nnH,
     &     neg, nfix(2), PrintL, PrintS, toldj1, toldj2, toldj3, typeLU
      double precision
     &     Anorm, Bgrwth, Bold, featol, condHz, djq, djqPrt, infBnd,
     &     ObjLP, ObjPrt, pivot, rowerr, sgnObj, sInfE,
...
!     ------------------------------------------------------------------
     ...
     &                      kSumm  .eq.  1                      )
      ...
      justPh1 = Prob .eq. FP .or.  Prob   .eq. FPE  .or.  Prob  .eq. FPS
...
!+    do while (.not. optiml  .and.  iExit .eq. 0)
  100 if       (.not. optiml  .and.  iExit .eq. 0) then
     ...
     &           ne, nlocA, locA, indA, Acol,
     &           hEtype, hs, bl, bu, dummy, pi, rc, x, xFreez,
     &           iw, leniw, rw, lenrw )
            optiml = nonOpt .eq. 0
         end if ! feasbl and justph1
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &     none
      integer
     &     nb, nBS, nElast, hEtype(nb), hEstat(nb), kBS(nBS)
      double precision
...
!     ==================================================================
!     s5Eset  resets the upper and lower bounds on the elastic variables
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &     hEtype(n), hs(n), locA(nlocA), indA(ne)
      double precision
...
!     ------------------------------------------------------------------
      do j = j1, j2
         if (hs(j) .eq. 4  .and.  hEtype(j) .gt. 0) then
            dj    = zero
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &     ne, nlocA, locA, indA, Acol,
     &     hEtype, hEstat, hrtype, hfeas, hs, kBS, Names,
     ...
     &     hEtype(nb), hEstat(nb), hfeas(mBS), hrtype(nb), hs(nb),
     &     kBS(mBS), locA(nlocA), indA(ne), iy(nb), iy1(nb), iw(leniw)
      double precision
...
!     ------------------------------------------------------------------
     ...
     &            ( m, n, nb, nnCon, nrhsLP,
     &              ne, nlocA, locA, indA, Acol,
     &              bl, bu, nrhs0, nrhs, rhs, y2, x, y, rw, lenrw )
            end if
            ...
            Elastc  = .false.
            lvlInf  =  0
            lEmode  =  0        ! No Elastic mode for E-row phase 1
            needLU  = .true.
            needx   =  needLU
            subopt  =  No
     ...
     &           hEtype, hEstat, hfeas, hs, kBS,
     &           Ascale, bl, bu, blBS, buBS,
     &           gBS, pi, rc, nrhsLP0, nrhsLP, y2, x, xBS, x,
     &           iy, iy1, y, y1,
     &           cw, lencw, iw, leniw, rw, lenrw )
...
!     ------------------------------------------------------------------
      ...
      parameter         (zero = 0.0d+0, one  = 1.0d+0)
!     ------------------------------------------------------------------
!     Find the number and sum of the nonelastic infeasibilities.
!     ------------------------------------------------------------------
      numInf = 0
      sumInf = zero
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &     leniw, lenrw, hfeas(m1), kBS(m1), hEtype(nb), hEstat(nb),
     &     hs(nb), iw(leniw)
      double precision
     &     featol, pivot, step, tolinc,
...
!     =================================================================
     ...
     &     hitlow, move, onbnd, unbndd
      integer
     &     inform, infpiv, jr, jrstat, js, LUmod
      double precision
     &     bound, exact, bigdx, infBnd, stepP, stepMax, tolpiv
!     ------------------------------------------------------------------
      integer            xBStox
      parameter         (xBStox = 1)
      double precision   zero,            one
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &     jq, djq, kPrc, toldj,
     &     ne, nlocA, locA, indA, Acol,
     &     hEtype, hs, bl, bu, g, pi, rc, x, xFreez,
     &     iw, leniw, rw, lenrw )
     ...
     &     hEtype(nb), hs(nb), locA(nlocA), indA(ne), iw(leniw)
      double precision
...
!     ==================================================================
     ...
     &     j, jtype, jhitI, jhitF
      double precision
     &     stepI, delta, pivot, pivabs, pivmaxI, pivmaxF, res, stepMin
!     ------------------------------------------------------------------
...
!                 The lower bound is satisfied. Test for smaller stepP.
...
!              x  is increasing.
!              The upper bound is satisfied. Test for smaller stepP
...
!     ------------------------------------------------------------------
!     Second pass.
!     For feasible variables, recompute the steps without  perturbation.
!     Choose  the largest  pivot element  subject to  the step  being no
!     greater than stepP.
!     For infeasible variables, find the largest steplength subject to
...
!                 Lower bound is satisfied. Test for a bigger pivot.

                  if (pivabs .gt. pivmaxF) then
                     res = xBS(j) - blBS(j)
...
!                    Upper bound is violated. Test for a bigger stepI.
...
!              x  is increasing.
!              Upper bound is satisfied.  Test for a bigger pivot
...
!                 Lower bound is violated. Test for a bigger stepI.
...
!           The steplength stepF is not used, so no need to get it,
...
!        ---------------------------------------------------------------
!        Try to step exactly onto a bound, but make sure the exact stepB
!        is  sufficiently  positive. (exact  will  be  stepF or  stepI.)
!        Since featol increases by tolinc each iteration, we know that a
!        step as  large as stepMin  (below) will not cause  any feasible
