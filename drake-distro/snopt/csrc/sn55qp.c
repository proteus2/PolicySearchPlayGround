...
/* Subroutine */ int s5qp_(integer *iexit, integer *prob, char *probtag, 
	...
	integer *nb, integer *ndegen, integer *hcalls, integer *ngqp0, 
...
/*                    =================================================== */
			if (gotgqp) {
			    ...
			    s5qpfg_((U_fp)hprod, (U_fp)hprod1, ngqp, ngobj0, 
				    ngobj, nnh, &sqstat, hcalls, objqp, &gobj[
...
/*           ------------------------------------------------------------ */
	    s5qpit_(&inform__, (U_fp)hprod, (U_fp)hprod1, &bndswp, elastc, &
		    ...
		    n, nb, hcalls, nnh0, nnh, ns, ngqp0, ngqp, ndegen, &lureq,
		     &kp, &jbq, &jsq, &jbr, &jsr, &jq, &jqsave, &nfmove, &
		    nuncon, &djq0, &djq, minimz, &obj, objqp, &featol, &pivot,
		     &step, &tolinc, wtinf, ne, nloca, &loca[1], &inda[1], &
		    acol[1], &hetype[1], &hestat[1], &hfeas[1], &hs[1], &kbs[
		    1], &bl[1], &bu[1], &blbs[1], &bubs[1], &gbs[1], &gqp[1], 
		    &hdx[1], &pbs[1], &rg[1], &r__[1], &x[1], &xbs[1], &y[1], 
		    &y1[1], &y2[1], cu + 8, lencu, &iu[1], leniu, &ru[1], 
...
/* Subroutine */ int s5getr_(integer *iexit, U_fp hprod, U_fp hprod1, integer 
	*hcalls, logical *gotr, integer *typelu, integer *lureq, integer *itn,
	 integer *lenr, integer *m, integer *mbs, integer *n, integer *nb, 
...
/*           ------------------------------------------------------------ */
	    s5hz_(&inform__, (U_fp)hprod, (U_fp)hprod1, &maxr, lenr, minimz, 
		    m, mbs, n, nb, nnh, ns, hcalls, ne, nloca, &loca[1], &
...
/* Subroutine */ int s5hz_(integer *iexit, S_fp hprod, U_fp hprod1, integer *
	...
	integer *n, integer *nb, integer *nnh, integer *ns, integer *hcalls, 
...
/* Set v = H w. */
	if (*nnh > 0) {
	    (*hprod)((U_fp)hprod1, hcalls, nnh, &w[1], &v[1], &sqstat, cu + 8,
...
/* Subroutine */ int s5qpfg_(S_fp hprod, U_fp hprod1, integer *ngqp, integer *
	...
	hcalls, doublereal *fqp, doublereal *gobj, doublereal *gqp, integer *
...
/*     ------------------------------------------------------------------ */
    ...
    *fqp = 0.;
    if (*nnh > 0) {
	(*hprod)((U_fp)hprod1, hcalls, nnh, &dx[1], &gqp[1], sqstat, cu + 8, 
...
/* Subroutine */ int s5qpit_(integer *iexit, S_fp hprod, U_fp hprod1, logical 
	*bndswp, logical *elastc, logical *feasbl, logical *gotgqp, logical *
	...
	integer *lenr, integer *m, integer *mbs, integer *maxr, integer *maxs,
	 integer *n, integer *nb, integer *hcalls, integer *nnh0, integer *
...
/*           ------------------------------------------------------------ */
	    ...
	    s5rcol_(iexit, (S_fp)hprod, (U_fp)hprod1, minimz, jq, &ns1, &drsq,
		     &ldiag, maxr, lenr, m, mbs, n, nb, hcalls, nnh, &ns1, ne,
		     nloca, &loca[1], &inda[1], &acol[1], &kbs[1], &r__[1], &
		    y[1], &y2[1], &pbs[1], cu + 8, lencu, &iu[1], leniu, &ru[
...
/* gotR */
	...
	if (rcheck && *gotr) {
	    s5rchk_(iexit, (S_fp)hprod, (U_fp)hprod1, itn, minimz, maxr, lenr,
		     m, mbs, n, nb, hcalls, nnh, &ns1, ne, nloca, &loca[1], &
...
/*        --------------------------------------------------------------- */
	if (*needf && (*gotgqp || *goth)) {
	    ...
	    if (*goth) {
		phpqp = 0.;
		(*hprod)((U_fp)hprod1, hcalls, nnh, &y[1], &hdx[1], &sqstat, 
...
/* Subroutine */ int s5rchk_(integer *iexit, S_fp hprod, U_fp hprod1, integer 
	*itn, integer *minimz, integer *maxr, integer *lenr, integer *m, 
	integer *mbs, integer *n, integer *nb, integer *hcalls, integer *nnh, 
...
/*        --------------------------------------------------------------- */
	...
	if (*nnh > 0) {
	    status = 0;
	    (*hprod)((U_fp)hprod1, hcalls, nnh, &w[1], &v[1], &status, cu + 8,
		     lencu, &iu[1], leniu, &ru[1], lenru, cw + 8, lencw, &iw[
		    1], leniw, &rw[1], lenrw, (ftnlen)8, (ftnlen)8);
...
/* Subroutine */ int s5rcol_(integer *iexit, S_fp hprod, U_fp hprod1, integer 
	*minimz, integer *jq, integer *jradd, doublereal *drsq, integer *
	...
	integer *n, integer *nb, integer *hcalls, integer *nnh, integer *ns, 
	integer *ne, integer *nloca, integer *loca, integer *inda, doublereal 
