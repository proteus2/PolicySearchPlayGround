/* ../src/sn20amat.f -- translated by f2c (version 20061008).
...
/* scale tolerance. */
    prnt1 = *lprint >= 10;
    if (prnt1) {
	snprnt_(&c__31, " ", &iw[1], leniw, (ftnlen)1);
	snprnt_(&c__21, " Scaling", &iw[1], leniw, (ftnlen)8);
	snprnt_(&c__21, " -------", &iw[1], leniw, (ftnlen)8);
	snprnt_(&c__21, "             Min elem    Max elem       Max col rat"
		"io", &iw[1], leniw, (ftnlen)53);
    }
    bplus = infbnd * .1;
    aratio = bplus;
    mxpass = 10;
...
/*     Remember, column scales are upside down. */
    ...
    amin = 1. / amin;
    if (prnt1) {
	snprnt_(&c__31, "            Min scale                       Max sca"
...
/* Computing MAX */
    ...
    if (lprscl > 0) {
	snprnt_(&c__31, " ", &iw[1], leniw, (ftnlen)1);
	snprnt_(&c__21, " Row scales  r(i)      a(i,j)  =  r(i) * scaled a(i"
		",j) / c(j)", &iw[1], leniw, (ftnlen)61);
	snprnt_(&c__21, " ----------------", &iw[1], leniw, (ftnlen)17);
	i__1 = *m;
	for (i__ = istart; i__ <= i__1; ++i__) {
	    s_wsfi(&io___207);
	...
	snprnt_(&c__31, " ", &iw[1], leniw, (ftnlen)1);
	snprnt_(&c__21, " Column scales  c(j)      x(j)    =  c(j) * scaled "
		"x(j)", &iw[1], leniw, (ftnlen)55);
	snprnt_(&c__21, " -------------------", &iw[1], leniw, (ftnlen)20);
	i__1 = *n;
	...
	*acol, integer *hs)
{
    ...
    static cilist io___230 = { 0, 0, 0, fmt_1000, 0 };
