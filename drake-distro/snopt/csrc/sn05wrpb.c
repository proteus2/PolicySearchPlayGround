/* ../src/sn05wrpb.f -- translated by f2c (version 20090411).
...
/* ================================================================= */
	...
	(*fgcon)(&modec, nncon, nnjac, negcon, &x[1], &fcon[1], &gcon[1], &
		status, cu + 8, lencu, &iu[1], leniu, &ru[1], lenru, (ftnlen)
		8);
	if (lvltim >= 2) {
...
/* ngObj = nnObj */
	    ...
	    (*fgobj)(&modef, ngobj, &x[1], fobj, &gobj[1], &status, cu + 8, 
		    lencu, &iu[1], leniu, &ru[1], lenru, (ftnlen)8);
	    if (lvltim >= 2) {
