	...
	ftnlen cw_len)
{
    ...
    extern /* Subroutine */ int s1file_(integer *, integer *, integer *), 
	    ...
	    *, U_fp, U_fp, U_fp, integer *, logical *, integer *, integer *, 
	    integer *, integer *, integer *, integer *, integer *, integer *, 
...
/*     ------------------------------------------------------------------ */
    nloca = *n + 1;
    s5solv_(info, solver, &iw[69], (U_fp)sqhx_, (U_fp)qphx, (U_fp)sqlog, &iw[
	    188], &gotr, m, n, &nb, &nnh0, &nnh, nname, &ngqp, &ngobj0, &
	    ngobj, iobj, objadd, &objqp, &objtru, ninf, sinf, ne, &nloca, &
	    loca[1], &inda[1], &acol[1], &bl[1], &bu[1], &cobj[1], names + 8, 
	    &nrhs0, &nrhs, rhs, &lenx0, &nx0, x0, &hetype[1], &hs[1], &x[1], &
	    pi[1], &rc[1], ns, cu + 8, lencu, &iu[1], leniu, &ru[1], lenru, 
	    cw + 8, lencw, &iw[1], leniw, &rw[1], lenrw, (ftnlen)6, (ftnlen)8,
	     (ftnlen)8, (ftnlen)8);
    *obj = objqp;
    *mincw = iw[47];
