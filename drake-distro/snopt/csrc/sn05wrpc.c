/* ../src/sn05wrpc.f -- translated by f2c (version 20090411).
	...
	doublereal *rw, integer *lenrw, ftnlen cu_len, ftnlen cw_len)
{
    ...
    /* Fortran I/O blocks */
    static icilist io___20 = { 0, str, 0, fmt_1100, 80, 1 };
    static icilist io___22 = { 0, str, 0, fmt_2010, 80, 1 };
    static icilist io___23 = { 0, str, 0, fmt_2000, 80, 1 };
    static icilist io___24 = { 0, str, 0, fmt_2100, 80, 1 };
...
/* record of unknown derivatives */
    lgsav = iw[339];
/* gSav(nnObj) copy of true objective gradient */
    gdummy = rw[69];
...
/* ================================================================= */
	...
	(*userfun)(&mode, &nnobj, nncon, nnjac, nnl, negcon, &x[1], fobj, &rw[
		lgsav], &fcon[1], &gcon[1], &status, cu + 8, lencu, &iu[1], 
...
/* nnObj = ngObj */
	(*userfun)(&mode, ngobj, nncon, nnjac, nnl, negcon, &x[1], fobj, &
		gobj[1], &fcon[1], &gcon[1], &status, cu + 8, lencu, &iu[1], 
		leniu, &ru[1], lenru, (ftnlen)8);
    }
    if (lvltim >= 2) {
