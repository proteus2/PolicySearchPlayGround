/* ../src/snoptc.f -- translated by f2c (version 20090411).
...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* Subroutine */ int snoptc_(char *start, integer *m, integer *n, integer *ne,
	 integer *nname, integer *nncon, integer *nnobj, integer *nnjac, 
	integer *iobj, doublereal *objadd, char *prob, U_fp userfun, 
	...
	cw_len)
{
    ...
    extern /* Subroutine */ int snkerc_(char *, integer *, integer *, integer 
	    *, integer *, integer *, integer *, integer *, integer *, 
	    doublereal *, char *, U_fp, U_fp, U_fp, U_fp, U_fp, doublereal *, 
...
/*     ------------------------------------------------------------------ */
    ...
    /* Function Body */
    snkerc_(start, m, n, ne, nname, nncon, nnobj, nnjac, iobj, objadd, prob, (
	    U_fp)userfun, (U_fp)snlog_, (U_fp)snlog2_, (U_fp)sqlog_, (U_fp)
	    snstop_, &jcol[1], &indj[1], &locj[1], &bl[1], &bu[1], names + 8, 
	    &hs[1], &x[1], &pi[1], &rc[1], info, mincw, miniw, minrw, ns, 
	    ninf, sinf, obj, cu + 8, lencu, &iu[1], leniu, &ru[1], lenru, cw 
	    + 8, lencw, &iw[1], leniw, &rw[1], lenrw, start_len, (ftnlen)8, (
...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* subroutine snOptc */
/* Subroutine */ int snkerc_(char *start, integer *m, integer *n, integer *ne,
	 integer *nname, integer *nncon, integer *nnobj, integer *nnjac, 
	integer *iobj, doublereal *objadd, char *prob, U_fp userfun, U_fp 
	snlog, U_fp snlog2, U_fp sqlog, U_fp snstop, doublereal *jcol, 
	integer *indj, integer *locj, doublereal *bl, doublereal *bu, char *
	...
	ftnlen names_len, ftnlen cu_len, ftnlen cw_len)
{
    ...
    static logical gotr;
    extern /* Subroutine */ int sqhx_(), s0fgc_();
    ...
    static integer nlocj, lgsav, maxcw, maxiw;
    ...
    extern /* Subroutine */ int s5solv_(integer *, char *, integer *, U_fp, 
	    U_fp, U_fp, integer *, logical *, integer *, integer *, integer *,
	     integer *, integer *, integer *, integer *, integer *, integer *,
	    ...
	    doublereal *, integer *, char *, integer *, integer *, integer *, 
	    doublereal *, integer *, ftnlen, ftnlen, ftnlen, ftnlen), s8solv_(
	    integer *, char *, integer *, U_fp, U_fp, U_fp, U_fp, U_fp, U_fp, 
    ...
    static integer negcon, nmajor;
    extern /* Subroutine */ int chcopy_(integer *, char *, integer *, char *, 
	    integer *, ftnlen, ftnlen);
    static integer minmax, inform__, mqnmod, lvlhes;
...
/* number of Hessian-vector prods */
/*     ------------------------------------------------------------------ */
/* Current Hessian type */
    ...
    /* Function Body */
    s_copy(solver, "SNOPTC", (ftnlen)6, (ftnlen)6);
    *info = 0;
/*     ------------------------------------------------------------------ */
...
/* position of the objective row in J */
	...
	*minrw = nextrw - 1;
	if (*minrw <= *lenrw) {
	    iw[298] = lx0;
/* x0(nnL)     = pp starting point */
	    iw[339] = lgsav;
/* gSav(nnObj) copy of obj gradient */
...
/* gObj(nnObj) = Objective gradient */
	...
	s5solv_(info, solver, &iw[69], (U_fp)sqhx_, (U_fp)s8qphx_, (U_fp)
		sqlog, &iw[188], &gotr, m, n, &nb, &nnh0, &nnh, nname, &ngqp, 
		&ngobj0, &ngobj, iobj, objadd, &fobj, &objtru, ninf, sinf, ne,
		 &nlocj, &locj[1], &indj[1], &jcol[1], &bl[1], &bu[1], &rw[
		lgobj], names + 8, &nrhs0, &nrhs, rhs, &lenx0, &nx0, x0, &iw[
		lhetyp], &hs[1], &x[1], &pi[1], &rc[1], ns, cu + 8, lencu, &
		iu[1], leniu, &ru[1], lenru, cw + 8, lencw, &iw[1], leniw, &
		rw[1], lenrw, (ftnlen)6, (ftnlen)8, (ftnlen)8, (ftnlen)8);
    } else {
...
/*        Define the type of initial Hessian. */
	...
	s8solv_(info, solver, &iw[69], (U_fp)s0fgc_, (U_fp)userfun, (U_fp)
		userfun, (U_fp)snlog, (U_fp)snlog2, (U_fp)snstop, &gotr, m, n,
