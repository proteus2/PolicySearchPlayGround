/* ../src/sn85hess.f -- translated by f2c (version 20090411).
...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* subroutine s8getH */
/* Subroutine */ int s8h0_(integer *htype, integer *nnh, doublereal *u0pre, 
...
/* Diagonal of BFGS Hessian */
	s9lmh0_(htype, nnh, &hcndbd, u0pre, &rw[lhd], &rw[lu0], &iw[381]);
    } else if (lvlhes == 1) {
/*        ----------------------- */
/*        Full-memory Hessian. */
/*        ----------------------- */
...
/* U(lenU), full-memory BFGS H = U'U */
	...
	s9fmh0_(htype, nnh, &hcndbd, u0pre, &rw[lhd], &lenu, &rw[lu], &iw[381]
		);
    }
    return 0;
} /* s8h0_ */
...
/* Subroutine */ int s8hqn_(integer *iexit, U_fp fgwrap, U_fp fgcon, U_fp 
	fgobj, logical *usefd, integer *htype, integer *qptype, integer *info,
...
/* Computing MIN */
	d__1 = max(*u0ii,.01);
	*u0ii = min(d__1,10.);
	s8h0_(htype, nnl, u0ii, &iw[1], leniw, &rw[1], lenrw);
    } else {
/*        =============================================================== */
...
/*           ------------------------------------------------------------ */
	    *nskip = 0;
	    ...
	    rydx = sqrt(ydx);
	    rdxhdx = sqrt(*dxhdx);
	    s8hupd_(&info[1], htype, nnl, u0ii, &u0scal, &rydx, &rdxhdx, &dx[
		    1], &hdx[1], &dg[1], &iw[1], leniw, &rw[1], lenrw);
	    if (*qptype == 0) {
		*gotr = *gotr && *nncon == 0 && *ns > 0 && *htype == 0 && *
			rtrmod < kfac;
	    } else {
		*gotr = *gotr && *ns > 0 && *htype == 0;
	    }
	    if (*gotr) {
...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* subroutine s8HQN */
/* Subroutine */ int s8hwrp_(S_fp hprod, integer *hcalls, integer *nnh, 
...
/*     ================================================================== */
    ...
    /* Function Body */
    (*hprod)(hcalls, nnh, &x[1], &hx[1], cw + 8, lencw, &iw[1], leniw, &rw[1],
	     lenrw, (ftnlen)8);
    return 0;
} /* s8hwrp_ */
...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* subroutine s8Hwrp */
/* Subroutine */ int s8hx_(integer *hcalls, integer *nnh, doublereal *x, 
	doublereal *hx, char *cw, integer *lencw, integer *iw, integer *leniw,
...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* subroutine s8xHx */
/* Subroutine */ int s8hupd_(integer *update, integer *htype, integer *nnh, 
...
/* sk's for BFGS products: (I + sk*vk') */
	lv = iw[402];
/* vk's for BFGS products: (I + sk*vk') */
	s9lmup_(update, htype, nnh, &mqnmod, &iw[381], &hcndbd, u0pre, u0scal,
		 rydx, rdxhdx, &rw[lhd], &hdx[1], &y[1], &dx[1], &rw[lu0], &
		rw[ls], &rw[lv]);
    } else if (lvlhes == 1) {
	mqnmod = iw[54];
/* (ge 0) max # of BFGS updates */
...
/* U(lenU), full-memory BFGS Hessian H = U'U */
	...
	s9fmup_(update, htype, nnh, &mqnmod, &iw[381], &hcndbd, u0pre, u0scal,
		 rydx, rdxhdx, &rw[lhd], &hdx[1], &y[1], &rw[ludx], &lenu, &
		rw[lu]);
    }
    return 0;
} /* s8hupd_ */
