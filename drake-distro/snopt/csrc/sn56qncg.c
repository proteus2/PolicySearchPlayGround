/* ../src/sn56qncg.f -- translated by f2c (version 20061008).
	...
	ftnlen cu_len, ftnlen cw_len)
{
    ...
    /* Fortran I/O blocks */
    static icilist io___22 = { 0, str, 0, fmt_1000, 120, 1 };
	 ...
	 integer *lenrw, ftnlen probtag_len, ftnlen cu_len, ftnlen cw_len)
{
    ...
    /* Fortran I/O blocks */
    static icilist io___104 = { 0, str, 0, fmt_1030, 120, 1 };
    static icilist io___107 = { 0, str, 0, fmt_1610, 120, 1 };
    static icilist io___108 = { 0, str, 0, fmt_1620, 120, 1 };
    static icilist io___117 = { 0, str, 0, fmt_1700, 120, 1 };
    static icilist io___120 = { 0, str, 0, fmt_8050, 120, 1 };
    static icilist io___121 = { 0, str, 0, fmt_8060, 120, 1 };
    static icilist io___122 = { 0, str, 0, fmt_1010, 120, 1 };
    static icilist io___123 = { 0, str, 0, fmt_1020, 120, 1 };
    static icilist io___143 = { 0, str, 0, fmt_1000, 120, 1 };
...
/*                 ====================================================== */
			...
			s5qpfg_((U_fp)hprod, (U_fp)hprod1, ngqp, ngobj0, 
				ngobj, nnh, &sqstat, hcalls, objqp, &gobj[1], 
				&gqp[1], lenx0, nx0, &x0[1], &x[1], &y[1], cu 
				+ 8, lencu, &iu[1], leniu, &ru[1], lenru, cw 
				+ 8, lencw, &iw[1], leniw, &rw[1], lenrw, (
				ftnlen)8, (ftnlen)8);
			sqstat = 0;
		    }
		    if (goth && needr && ! (*gotr)) {
...
/*              --------------------------------------------------------- */
	    s5qnit_(&inform__, (U_fp)hprod, (U_fp)hprod1, elastc, &feasbl, &
		    ...
		    newsb, itn, lenr, m, mbs, &maxr, maxs, n, nb, hcalls, 
...
/* Subroutine */ int s5qnit_(integer *iexit, S_fp hprod, U_fp hprod1, logical 
	*elastc, logical *feasbl, logical *gotgqp, logical *goth, logical *
	...
	integer *hcalls, integer *nnh0, integer *nnh, integer *ns, integer *
...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
/* subroutine s5Hzx */
/* Subroutine */ int s5hzx1_(S_fp hprod, U_fp hprod1, integer *hcalls, 
...
/*     Compute  y1 = - S*yS. */
    s2bprd_(&c__0, &eps0, n, ns, &kbs[m1], ne, nloca, &loca[1], &inda[1], &
	    acol[1], &c_b12, &y[m1], ns, &c_b26, &y1[1], m);
/*     Solve  B*y = y1. */
...
/*    &   ( Hcalls, nnH, y1, y, cw, lencw, iw, leniw, rw, lenrw ) */
    (*hprod)((U_fp)hprod1, hcalls, nnh, &y1[1], &y[1], &sqstat, cu + 8, lencu,
	     &iu[1], leniu, &ru[1], lenru, cw + 8, lencw, &iw[1], leniw, &rw[
...
/*     Set ZHZx = y1S - S'y. */
    ...
    s2bprd_(&c__1, &eps0, n, ns, &kbs[m1], ne, nloca, &loca[1], &inda[1], &
	    acol[1], &c_b12, &y[1], m, &c_b102, &zhzx[1], ns);
    if (precon == 1) {
