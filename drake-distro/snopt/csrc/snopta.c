/* ../src/snopta.f -- translated by f2c (version 20090411).
	...
	fnames_len, ftnlen cu_len, ftnlen cw_len)
{
    ...
    static integer mqnmodtmp, lvlhestmp, nb, ne, lx, nx0, lbl, lrc, lbu, nnh, 
	    lpi, lhs, lfx;
    ...
    static integer nnjac, lgobj, lindg, nname, lindj, llocg, ljcol, llocj, 
	    nnobj, nncon, maxcw, nlocj, nlocg, maxiw, maxrw;
    extern /* Subroutine */ int s2mem0_(integer *, char *, integer *, integer 
	    ...
	    *, integer *, integer *, integer *, integer *, integer *, ftnlen),
	    ...
	    ), s3blda_(integer *, integer *, integer *, integer *, integer *, 
	    integer *, integer *, integer *, integer *, integer *, integer *, 
...
/*     ------------------------------------------------------------------ */
    dload_(&nnobj, &c_b18, &rw[lgobj], &c__1);
    dload_(&nnobj, &c_b18, &rw[lgobj1], &c__1);
    dload_(&nnobj, &c_b18, &rw[lgobj2], &c__1);
/*     ------------------------------------------------------------------ */
...
/* No constant shift for x. */
	...
	s5solv_(info, solver, &iw[69], (U_fp)sqhx_, (U_fp)s8qphx_, (U_fp)
		sqlog, &iw[188], &gotr, &m, n, &nb, &nnh0, &nnh, &nname, &
		ngqp, &nnobj0, &nnobj, &iobj, objadd, &fobj, &objtru, ninf, 
		sinf, &ne, &nlocj, &iw[llocj], &iw[lindj], &rw[ljcol], &rw[
		lbl], &rw[lbu], &rw[lgobj], cw + (lnames << 3), &nrhs0, &nrhs,
		 rhs, &lenx0, &nx0, x0, &iw[lhetyp], &iw[lhs], &rw[lx], &rw[
		lpi], &rw[lrc], ns, cu + 8, lencu, &iu[1], leniu, &ru[1], 
		lenru, cw + 8, lencw, &iw[1], leniw, &rw[1], lenrw, (ftnlen)6,
