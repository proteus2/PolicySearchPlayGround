...
{
  fprintf(stderr,"****************************************************\n");
  fprintf(stderr,"Error: %s must be set prior to call to\n", var);
  fprintf(stderr,"snoptProblem::solve() or snoptProblem::computeJac()!\n");
  fprintf(stderr,"****************************************************\n");
  exit(1);
}
...
void snoptProblem::solve( integer starttype )
{
  ...
  if ( neA == -1 | neG == -1 ) {
    fprintf(stderr,"Warning: neA and neG must be set before calling snoptProblem::solve()\n");
    exit(1);
  }
