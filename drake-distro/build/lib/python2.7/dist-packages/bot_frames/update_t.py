"""LCM type definitions
This file automatically generated by lcm.
DO NOT MODIFY BY HAND!!!!
"""

import cStringIO as StringIO
import struct

class update_t(object):
    __slots__ = ["utime", "frame", "relative_to", "trans", "quat"]

    def __init__(self):
        self.utime = 0
        self.frame = ""
        self.relative_to = ""
        self.trans = [ 0.0 for dim0 in range(3) ]
        self.quat = [ 0.0 for dim0 in range(4) ]

    def encode(self):
        buf = StringIO.StringIO()
        buf.write(update_t._get_packed_fingerprint())
        self._encode_one(buf)
        return buf.getvalue()

    def _encode_one(self, buf):
        buf.write(struct.pack(">q", self.utime))
        __frame_encoded = self.frame.encode('utf-8')
        buf.write(struct.pack('>I', len(__frame_encoded)+1))
        buf.write(__frame_encoded)
        buf.write("\0")
        __relative_to_encoded = self.relative_to.encode('utf-8')
        buf.write(struct.pack('>I', len(__relative_to_encoded)+1))
        buf.write(__relative_to_encoded)
        buf.write("\0")
        buf.write(struct.pack('>3d', *self.trans[:3]))
        buf.write(struct.pack('>4d', *self.quat[:4]))

    def decode(data):
        if hasattr(data, 'read'):
            buf = data
        else:
            buf = StringIO.StringIO(data)
        if buf.read(8) != update_t._get_packed_fingerprint():
            raise ValueError("Decode error")
        return update_t._decode_one(buf)
    decode = staticmethod(decode)

    def _decode_one(buf):
        self = update_t()
        self.utime = struct.unpack(">q", buf.read(8))[0]
        __frame_len = struct.unpack('>I', buf.read(4))[0]
        self.frame = buf.read(__frame_len)[:-1].decode('utf-8', 'replace')
        __relative_to_len = struct.unpack('>I', buf.read(4))[0]
        self.relative_to = buf.read(__relative_to_len)[:-1].decode('utf-8', 'replace')
        self.trans = struct.unpack('>3d', buf.read(24))
        self.quat = struct.unpack('>4d', buf.read(32))
        return self
    _decode_one = staticmethod(_decode_one)

    _hash = None
    def _get_hash_recursive(parents):
        if update_t in parents: return 0
        tmphash = (0xe81404582309d273) & 0xffffffffffffffff
        tmphash  = (((tmphash<<1)&0xffffffffffffffff)  + (tmphash>>63)) & 0xffffffffffffffff
        return tmphash
    _get_hash_recursive = staticmethod(_get_hash_recursive)
    _packed_fingerprint = None

    def _get_packed_fingerprint():
        if update_t._packed_fingerprint is None:
            update_t._packed_fingerprint = struct.pack(">Q", update_t._get_hash_recursive([]))
        return update_t._packed_fingerprint
    _get_packed_fingerprint = staticmethod(_get_packed_fingerprint)

